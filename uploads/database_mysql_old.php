...
class Database {
    ...
    public function getCityComments($cityId = null, $status = null) {
        if (!$this->isConnected()) { return []; }
        $sql = 'SELECT c.*, ci.name as city_name FROM comments c LEFT JOIN cities ci ON c.city_id = ci.id WHERE c.city_id IS NOT NULL';
        $params = [];
        if ($cityId) {
            $sql .= ' AND c.city_id = ?';
            $params[] = $cityId;
    ...
    public function createCityComment($city_id, $author_name, $author_email, $content, $rating = null) {
        if (!$this->isConnected()) { return false; }
        $status = 'pending';
        $sql = "INSERT INTO comments (city_id, author_name, author_email, content, rating, status, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())";
        $stmt = $this->pdo->prepare($sql);
