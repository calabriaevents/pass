...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($entity === 'gallery') {
        $redirect_url .= 'entity=gallery&action=manage&province_id=' . ($_POST['province_id'] ?? '');
    }
...
if ($action === 'delete' && $id) {
    if ($entity === 'provinces') {
        ...
        // Qui dovremmo anche eliminare le immagini della galleria associate
        $gallery_images = $db->getProvinceGalleryImages($id);
        foreach($gallery_images as $img) {
            $imageProcessor->deleteImage($img['image_path']);
        }
    ...
    $redirect_url = 'province.php?';
     if ($entity === 'gallery') {
        $redirect_url .= 'entity=gallery&action=manage&province_id=' . ($_GET['province_id'] ?? '');
    }
    header('Location: ' . $redirect_url);
    exit;
}
...
<body class="min-h-screen bg-gray-100 flex">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <main class="flex-1 overflow-auto p-6">
            <?php if ($entity === 'provinces' && ($action === 'list' || $action === '')): ?>
                <!-- Elenco Province -->
                    ...
                    <table class="w-full">
                        ...
                        <tbody>
                            ...
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="py-3 px-2"><?php echo htmlspecialchars($province['name']); ?></td>
                                <td class="py-3 px-2">
                                    <a href="?entity=provinces&action=edit&id=<?php echo $province['id']; ?>" class="text-blue-600 hover:underline">Modifica</a>
                                    <a href="?entity=gallery&action=manage&province_id=<?php echo $province['id']; ?>" class="text-green-600 hover:underline ml-2">Galleria</a>
                                    <a href="?entity=provinces&action=delete&id=<?php echo $province['id']; ?>" onclick="return confirm('Sei sicuro?');" class="text-red-600 hover:underline ml-2">Elimina</a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
            ...
            <?php elseif ($entity === 'provinces' && ($action === 'new' || $action === 'edit')):
                $province = null;
                if ($action === 'edit' && $id) {
                    $province = $db->getProvinceById($id);
                }
            ?>
                <!-- Form Provincia -->
                <h2 class="text-lg font-semibold mb-4"><?php echo $action === 'edit' ? 'Modifica' : 'Nuova'; ?> Provincia</h2>
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <form action="?entity=provinces&action=<?php echo $action; ?><?php if ($id) echo '&id='.$id; ?>" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                           ...
                           <?php if ($province && $province['image_path']): ?>
                           <div class="mb-2">
                               <p>Immagine attuale:</p>
                               <!-- MODIFICATO PER USARE IMAGE-LOADER.PHP -->
                               <img src="../image-loader.php?path=<?php echo urlencode($province['image_path']); ?>" alt="Immagine attuale" class="max-w-xs rounded-lg shadow">
                           </div>
                           <?php endif; ?>
            ...
            <?php elseif ($entity === 'gallery' && $action === 'manage' && isset($_GET['province_id'])):
            ...
            ?>
                <!-- Gestione Galleria -->
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-lg font-semibold">Galleria per <?php echo htmlspecialchars($province['name']); ?></h2>
                    <a href="?entity=gallery&action=add&province_id=<?php echo $province_id; ?>" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg">Aggiungi Immagine</a>
                </div>
                <div class="bg-white rounded-lg shadow-sm p-6 grid grid-cols-4 gap-4">
                    <?php foreach ($gallery_images as $image): ?>
                        <div class="relative">
                            <!-- MODIFICATO PER USARE IMAGE-LOADER.PHP -->
                            <img src="../image-loader.php?path=<?php echo urlencode($image['image_path']); ?>" alt="<?php echo htmlspecialchars($image['title']); ?>" class="w-full h-48 object-cover rounded-lg">
                            <a href="?entity=gallery&action=delete&id=<?php echo $image['id']; ?>&province_id=<?php echo $province_id; ?>" onclick="return confirm('Sei sicuro?');" class="absolute top-2 right-2 bg-red-600 text-white rounded-full p-1">&times;</a>
                            <div class="mt-2">
                               <p class="font-bold"><?php echo htmlspecialchars($image['title']); ?></p>
            ...
            <?php elseif ($entity === 'gallery' && $action === 'add' && isset($_GET['province_id'])):
                $province_id = (int)$_GET['province_id'];
            ?>
                <!-- Form Aggiungi Immagine Galleria -->
                <h2 class="text-lg font-semibold mb-4">Aggiungi Immagine a Galleria</h2>
                <div class="bg-white rounded-lg shadow-sm p-6">
                     <form action="?entity=gallery&action=add" method="POST" enctype="multipart/form-data">
