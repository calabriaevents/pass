...
try {
    ...
    $stmt = $db->pdo->query("SELECT COUNT(*) as count FROM subscriptions WHERE status = 'active' AND end_date <= DATE_ADD(NOW(), INTERVAL 30 DAY) AND end_date > NOW()");
    $stats['expiring'] = $stmt->fetch()['count'];
    
    // Expired
    $stmt = $db->pdo->query("SELECT COUNT(*) as count FROM subscriptions WHERE status = 'expired' OR (status = 'active' AND end_date < NOW())");
...
if ($filter !== 'all') {
    switch ($filter) {
        case 'active':
            $whereConditions[] = "s.status = 'active' AND (s.end_date IS NULL OR s.end_date > NOW())";
            break;
        case 'expiring':
            $whereConditions[] = "s.status = 'active' AND s.end_date <= DATE_ADD(NOW(), INTERVAL 30 DAY) AND s.end_date > NOW()";
            break;
        case 'expired':
            $whereConditions[] = "(s.status = 'expired' OR (s.status = 'active' AND s.end_date < NOW()))";
            break;
        case 'cancelled':
...
// Helper function for date formatting - Removed duplicate, using the one from config.php
?>
<!DOCTYPE html>
<html lang="it">
<head>
