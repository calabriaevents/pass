...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $consumption_credits = intval($_POST['consumption_credits'] ?? 0);
    ...
    try {
        if ($action === 'create') {
            ...
            $stmt = $db->pdo->prepare('
                ...
                WHERE id = ? AND package_type = "consumption"
            ');
            $stmt->execute([$name, $description, $price, $consumption_credits, $features_json, $stripe_price_id, $is_active, $sort_order, $image_path, $id]);
            header('Location: consumo-pacchetti.php?message=updated');
        }
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
...
if ($action === 'delete' && $id) {
    try {
        // Check if package is used in consumption purchases
        $stmt = $db->pdo->prepare('SELECT COUNT(*) as count FROM consumption_purchases WHERE package_id = ?');
        $stmt->execute([$id]);
        $usageCount = $stmt->fetch()['count'];
        
        if ($usageCount > 0) {
            header('Location: consumo-pacchetti.php?message=delete_error&used_count=' . $usageCount);
        } else {
            ...
            $stmt = $db->pdo->prepare('DELETE FROM business_packages WHERE id = ? AND package_type = "consumption"');
            $stmt->execute([$id]);
            header('Location: consumo-pacchetti.php?message=deleted');
        }
    } catch (Exception $e) {
        header('Location: consumo-pacchetti.php?message=error');
    }
    exit;
}
...
if ($action === 'toggle_status' && $id) {
    try {
        $stmt = $db->pdo->prepare('UPDATE business_packages SET is_active = NOT is_active WHERE id = ? AND package_type = "consumption"');
        $stmt->execute([$id]);
        header('Location: consumo-pacchetti.php?message=status_updated');
    } catch (Exception $e) {
        header('Location: consumo-pacchetti.php?message=error');
    }
    exit;
...
$packages = [];
try {
    $stmt = $db->pdo->prepare('SELECT * FROM business_packages WHERE package_type = "consumption" ORDER BY sort_order ASC, created_at ASC');
    $stmt->execute();
...
$stats = [];
try {
    $stmt = $db->pdo->query('SELECT COUNT(*) as total FROM business_packages WHERE package_type = "consumption"');
    $stats['total'] = $stmt->fetch()['total'];
    
    $stmt = $db->pdo->query('SELECT COUNT(*) as active FROM business_packages WHERE package_type = "consumption" AND is_active = 1');
    $stats['active'] = $stmt->fetch()['active'];
    
    $stmt = $db->pdo->query('SELECT COUNT(*) as purchased FROM consumption_purchases WHERE status = "completed"');
    $stats['purchased'] = $stmt->fetch()['purchased'];
    
    $stmt = $db->pdo->query('SELECT SUM(amount_paid) as revenue FROM consumption_purchases WHERE status = "completed"');
    $stats['revenue'] = $stmt->fetch()['revenue'] ?: 0;
} catch (Exception $e) {
    $stats = ['total' => 0, 'active' => 0, 'purchased' => 0, 'revenue' => 0];
}
?>
<!DOCTYPE html>
<html lang="it">
<head>
    ...
    <title>Pacchetti a Consumo - Admin Panel</title>
...
<body class="min-h-screen bg-gray-100 flex">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 flex items-center">
                        <i data-lucide="zap" class="w-7 h-7 text-orange-600 mr-2"></i>
                        Pacchetti a Consumo
                    </h1>
                    <p class="text-sm text-gray-500">Gestisci i pacchetti di crediti per servizi premium</p>
                </div>
                <div class="flex items-center space-x-4">
                    <a href="gestione-pacchetti.php" class="text-gray-600 hover:text-gray-800 px-4 py-2 rounded-lg border hover:bg-gray-50 transition-colors flex items-center">
                        <i data-lucide="package" class="w-4 h-4 mr-2"></i>
                        Pacchetti Abbonamento
                    </a>
                    <?php if ($action !== 'create' && $action !== 'edit'): ?>
                    <a href="consumo-pacchetti.php?action=create" class="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg font-medium transition-colors flex items-center">
        ...
        <main class="flex-1 overflow-auto p-6">
            ...
            <?php if (isset($_GET['message'])): ?>
                <?php
                $messages = [
                    'created' => ['type' => 'success', 'text' => '✅ Pacchetto a consumo creato con successo!'],
                    'updated' => ['type' => 'success', 'text' => '✅ Pacchetto a consumo aggiornato con successo!'],
                    'deleted' => ['type' => 'success', 'text' => '✅ Pacchetto a consumo eliminato con successo!'],
                    'status_updated' => ['type' => 'success', 'text' => '✅ Stato pacchetto aggiornato!'],
                    'delete_error' => ['type' => 'error', 'text' => '❌ Impossibile eliminare: pacchetto utilizzato in ' . ($_GET['used_count'] ?? 0) . ' acquisti'],
            ...
            <?php if ($action === 'list'): ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    ...
                    <div class="bg-white rounded-lg shadow-sm p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-medium text-gray-600">Acquisti Completati</p>
                                <p class="text-3xl font-bold text-blue-900"><?php echo $stats['purchased']; ?></p>
                            </div>
                    ...
                    <div class="bg-white rounded-lg shadow-sm p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-3xl font-bold text-purple-900">€<?php echo number_format($stats['revenue'], 2); ?></p>
                            </div>
                ...
                <div class="bg-white rounded-lg shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-200">
                        <h2 class="text-lg font-semibold flex items-center">
                            <i data-lucide="grid-3x3" class="w-5 h-5 mr-2 text-orange-600"></i>
                            Pacchetti a Consumo
                                ...
                                <div class="border rounded-xl p-6 <?php echo $package['is_active'] ? 'border-gray-200 bg-white' : 'border-gray-100 bg-gray-50'; ?> hover:shadow-lg transition-all">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div class="text-lg text-orange-600 font-semibold">
                                            <?php echo $package['consumption_credits']; ?> crediti
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                        ...
                                        <div class="flex items-center space-x-2">
                                            <a href="consumo-pacchetti.php?action=edit&id=<?php echo $package['id']; ?>" 
                                               class="text-blue-600 hover:text-blue-700 p-2 rounded-lg hover:bg-blue-50 transition-colors" title="Modifica">
                                                <i data-lucide="edit" class="w-4 h-4"></i>
                                            </a>
                                            
                                            <a href="consumo-pacchetti.php?action=toggle_status&id=<?php echo $package['id']; ?>" 
                                            ...
                                            <a href="consumo-pacchetti.php?action=delete&id=<?php echo $package['id']; ?>" 
                                               class="text-red-600 hover:text-red-700 p-2 rounded-lg hover:bg-red-50 transition-colors" title="Elimina"
                                               onclick="return confirm('Sei sicuro di voler eliminare questo pacchetto a consumo?')">
                    ...
                    <?php else: ?>
                        ...
                        <div class="text-center py-12">
                            <i data-lucide="zap" class="w-12 h-12 text-gray-300 mx-auto mb-4"></i>
                            <h3 class="text-lg font-medium text-gray-900 mb-2">Nessun pacchetto a consumo presente</h3>
                            <p class="text-gray-500 mb-6">Crea il primo pacchetto di crediti per servizi premium.</p>
                            <a href="consumo-pacchetti.php?action=create" class="inline-flex items-center px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors">
                                <i data-lucide="plus" class="w-4 h-4 mr-2"></i>
                ...
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <div class="mb-6">
                        <h2 class="text-xl font-bold text-gray-900 flex items-center mb-2">
                            <i data-lucide="<?php echo $action === 'create' ? 'plus' : 'edit'; ?>" class="w-6 h-6 mr-2 text-orange-600"></i>
                            <?php echo $action === 'create' ? 'Crea Nuovo Pacchetto a Consumo' : 'Modifica Pacchetto a Consumo'; ?>
                        </h2>
                        <p class="text-gray-600">Configura un pacchetto di crediti per servizi premium senza abbonamento mensile.</p>
                    </div>

                    <form action="consumo-pacchetti.php?action=<?php echo $action; ?><?php if ($id) echo '&id='.$id; ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <!-- Left Column -->
                            <div class="space-y-6">
                                <!-- Basic Info -->
                                <div class="bg-gray-50 p-6 rounded-lg">
                                    <h3 class="text-lg font-semibold mb-4">ℹ️ Informazioni Base</h3>
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <input type="text" name="name" id="name" required
                                                   ...
                                                   placeholder="es. Pacchetto Boost Pro">
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <textarea name="description" id="description" rows="3" required
                                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                                                      placeholder="Descrizione del pacchetto e dei suoi vantaggi"><?php echo htmlspecialchars($editPackage['description'] ?? ''); ?></textarea>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                <label for="consumption_credits" class="block text-sm font-medium text-gray-700 mb-2">Crediti Inclusi *</label>
                                                <input type="number" name="consumption_credits" id="consumption_credits" min="1" required
                                                       value="<?php echo $editPackage['consumption_credits'] ?? '0'; ?>"
                                                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent">
                                            </div>
                                        </div>
                            ...
                            <div class="space-y-6">
                                <!-- Features -->
                                <div class="bg-gray-50 p-6 rounded-lg">
                                    <h3 class="text-lg font-semibold mb-4">✨ Funzionalità e Vantaggi</h3>
                                    ...
                                    <div>
                                        ...
                                        <textarea name="features" id="features" rows="8"
                                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                                                  placeholder="Una funzionalità per riga, es:&#10;50 crediti per promozioni&#10;Evidenzia la tua attività&#10;Boost temporaneo nelle ricerche&#10;Validità 6 mesi"><?php 
                                            if ($editPackage) {
                                ...
                                <div class="bg-gray-50 p-6 rounded-lg">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500 mt-1">ID del prezzo creato in Stripe Dashboard per pagamento singolo</p>
                                    </div>
                                </div>

                                <!-- Status -->
                                <div class="bg-gray-50 p-6 rounded-lg">
                                    <h3 class="text-lg font-semibold mb-4">⚡ Stato</h3>
                        ...
                        <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                            <a href="consumo-pacchetti.php" class="text-gray-600 hover:text-gray-800 font-medium">
                                ← Torna alla lista
                            </a>
                            <div class="flex items-center space-x-4">
                                <a href="consumo-pacchetti.php" class="px-4 py-2 text-gray-600 hover:text-gray-800 font-medium">
                                    Annulla
                                </a>
    ...
    <script>
        ...
        // Auto-calculate credits/euro ratio
        const priceInput = document.getElementById('price');
        const creditsInput = document.getElementById('consumption_credits');
        
        function updateRatio() {
            if (priceInput && creditsInput && priceInput.value && creditsInput.value) {
                const price = parseFloat(priceInput.value);
                const credits = parseInt(creditsInput.value);
                const ratio = (price / credits).toFixed(3);
                console.log(`Rapporto: €${ratio} per credito`);
            }
        }
        
        priceInput?.addEventListener('input', updateRatio);
        creditsInput?.addEventListener('input', updateRatio);
        ...
        document.getElementById('consumption_credits')?.addEventListener('change', function() {
